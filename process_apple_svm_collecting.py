...
class Process(object):
    ...
    #             max_frequency = np.argmax(sample_fft[j][0:(int)(SAMPLE_LEN/2)])
...
print(left_start, left.time[left_start], right_start, right.time[right_start])
while left_end < len(left.time) and right_end < len(right.time):
    #show current 50 frame
    left_data = {}
    right_data = {}
    left_time = left.time[left_start:left_end]
    right_time = right.time[right_start:right_end]
    left_data['acc'] = left.data['acc'][left_start:left_end]
    left_data['att'] = left.data['att'][left_start:left_end]
    left_data['rot'] = left.data['rot'][left_start:left_end]
    right_data['acc'] = right.data['acc'][right_start:right_end]
    right_data['att'] = right.data['att'][right_start:right_end]
    right_data['rot'] = right.data['rot'][right_start:right_end]
    display_fig, display_axs = plt.subplots(9, 2)
    print(len([data[0] for data in left_data['acc']]))
    print(len(left_time))
    # plt.axis([min(left_time[0], right_time[0]), max(left_time[-1], right_time[-1]), -10, 10])
    display_axs[0, 0].plot(left_time, [data[0] for data in left_data['acc']])
    display_axs[1, 0].plot(left_time, [data[1] for data in left_data['acc']])
    display_axs[2, 0].plot(left_time, [data[2] for data in left_data['acc']])
    display_axs[3, 0].plot(left_time, [data[0] for data in left_data['att']])
    display_axs[4, 0].plot(left_time, [data[1] for data in left_data['att']])
    display_axs[5, 0].plot(left_time, [data[2] for data in left_data['att']])
    display_axs[6, 0].plot(left_time, [data[0] for data in left_data['rot']])
    display_axs[7, 0].plot(left_time, [data[1] for data in left_data['rot']])
    display_axs[8, 0].plot(left_time, [data[2] for data in left_data['rot']])
    display_axs[0, 1].plot(right_time, [data[0] for data in right_data['acc']])
    display_axs[1, 1].plot(right_time, [data[1] for data in right_data['acc']])
    display_axs[2, 1].plot(right_time, [data[2] for data in right_data['acc']])
    display_axs[3, 1].plot(right_time, [data[0] for data in right_data['att']])
    display_axs[4, 1].plot(right_time, [data[1] for data in right_data['att']])
    display_axs[5, 1].plot(right_time, [data[2] for data in right_data['att']])
    display_axs[6, 1].plot(right_time, [data[0] for data in right_data['rot']])
    display_axs[7, 1].plot(right_time, [data[1] for data in right_data['rot']])
    display_axs[8, 1].plot(right_time, [data[2] for data in right_data['rot']])
    # plt.ylim(-10, 10)
    plt.show()
    ...
    if 'y' in operation:
        ...
        #move
        step = int(str(operation).split()[1])
        left_start = left_start + step
        right_start = right_start + step
        left_end = left_end + step
        right_end = right_end + step
    else:
        print('input error')
